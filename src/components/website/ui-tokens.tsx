...
export function BadgeVersion({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={`bg-zinc-100/10 text-zinc-100 border border-zinc-800 px-2 py-0.5 rounded text-[11px] font-mono font-bold tabular-nums tracking-normal ${className}`}>
      {children}
    </span>
  );
}
...
export function CliCopyButton({ command = "npx gitforprompts init" }: { command?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-emerald-400 font-mono font-bold tracking-normal whitespace-nowrap overflow-x-auto scrollbar-none text-[11px] sm:text-xs">$ {command}</span>
...
export function BadgePastel({ children, variant = "blue", className = "" }: { children: React.ReactNode; variant?: "blue" | "green" | "yellow" | "rose"; className?: string }) {
  const variantStyles = {
    blue: "bg-bg-panel border border-zinc-800 text-zinc-300",
