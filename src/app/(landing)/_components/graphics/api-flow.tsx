...
export function ApiFlowGraphic() {
  return (
    <div className="flex-1 flex flex-col p-5 justify-between h-full animate-in fade-in duration-400 relative">
      {/* Local styles for precise packet flow and node pulse animations */}
      <style>{`
        ...
        @keyframes packet-req-local {
          ...
          40%  { left: calc(100% - 36px); opacity: 1; }
          45%  { left: calc(100% - 36px); opacity: 0; }
      ...
      <div className="flex items-center justify-between border-b border-zinc-900 pb-3 z-10">
        <span className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-semibold">Runtime API Delivery</span>
        <span className="text-xs font-mono text-zinc-400 font-semibold">● Live endpoint</span>
      </div>
      ...
      <div className="flex items-center gap-3 px-2 py-5 z-10">
        ...
        <div className="shrink-0 p-3.5 border border-zinc-800 rounded-xl bg-zinc-950 flex flex-col items-center w-28 shadow-lg shadow-zinc-950/20 animate-client-node-local">
          ...
          <span className="text-xs font-mono text-zinc-400 uppercase font-bold tracking-wider">Your App</span>
          <span className="text-xs text-zinc-300 font-mono mt-0.5 font-bold">Backend</span>
        </div>
        ...
        <div className="flex-1 relative h-10 flex items-center">
          ...
          <div className="animate-packet-req-local absolute top-1/2 -translate-y-1/2 flex items-center gap-1">
            ...
            <span className="text-xs font-mono text-sky-400 font-bold whitespace-nowrap">GET</span>
          </div>
          ...
          <div className="animate-packet-res-local absolute top-1/2 -translate-y-1/2 flex items-center gap-1">
            <span className="text-xs font-mono text-emerald-400 font-bold whitespace-nowrap">200</span>
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" style={{ boxShadow: '0 0 8px rgba(52,211,153,0.9)' }} />
          </div>

          {/* Track label */}
          <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 text-xs font-mono text-zinc-400 whitespace-nowrap font-semibold">
        ...
        <div className="shrink-0 p-3.5 border border-emerald-900/60 rounded-xl bg-zinc-950 flex flex-col items-center w-28 shadow-lg shadow-zinc-950/20 animate-gfp-node-local">
          <div className="w-5 h-5 mb-1.5 rounded bg-emerald-950/40 border border-emerald-900/40 flex items-center justify-center">
            <span className="text-xs font-mono text-emerald-400 font-bold">GFP</span>
          </div>
          <span className="text-xs font-mono text-emerald-500 uppercase font-bold tracking-wider">Edge CDN</span>
          <span className="text-xs text-emerald-300 font-mono mt-0.5 font-bold">returns_v2</span>
        </div>
      </div>

      {/* Response JSON panel */}
      <div className="border border-zinc-900 bg-zinc-950 rounded-lg overflow-hidden font-mono text-xs z-10 shadow-inner">
        <div className="px-3 py-1.5 border-b border-zinc-900 bg-zinc-900/40 flex justify-between items-center">
          <span className="text-zinc-400 text-xs font-mono">GET /api/v1/prompts/<span className="text-sky-400 font-mono">p_returns</span>/latest</span>
          <span className="text-xs font-bold text-emerald-400 font-mono bg-emerald-950/20 px-2 py-0.5 rounded border border-emerald-900/30">200 OK · 14ms</span>
        </div>
        <pre className="p-3 overflow-x-auto select-none leading-[1.7] text-xs font-mono">
          <span className="text-zinc-500">{'{'}</span>{`
  `}<span className="text-sky-400">&quot;id&quot;</span><span className="text-zinc-500">:</span> <span className="text-amber-300">&quot;p_customer_returns&quot;</span><span className="text-zinc-500">,</span>{`
  `}<span className="text-sky-400">&quot;version&quot;</span><span className="text-zinc-500">:</span> <span className="text-violet-400">2</span><span className="text-zinc-500">,</span>{`
